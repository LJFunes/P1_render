...
# @app.get('/')
# def mensaje():
#     return "MVP para consultar base de datos de STEAM y probar un modelo de recomendacion item-item"
...
def mensaje():
    ...
    <html>
        <head>
            <title>MVP - Steam&reg</title>
        </head>
        <body>
            <h1>Proyecto MVP (Minimum Viable Product) - Base de datos de STEAM&reg</h1>
            <p>En este MVP podrá realizar distintas consultas en la base de datos, y probar un modelo de recomendación item-item.</p>
            <p>Lista de los endpoints (con una opción de ejemplo para probar los mismos) y los datos que debe devolver.</p>
            <ul>
                <li><strong>developer</strong>(developer - ej: valve): devuelve cantidad de items y porcentaje de contenido Free por año según empresa desarrolladora </li>
                <li><strong>user_data</strong>(user_id - ej: 76561198002170343): devuelve cantidad de dinero gastado por el usuario, el porcentaje de recomendación en base a reviews.recommend y cantidad de items</li>
                <li><strong>user_for_genre</strong>(genre - ej: action): devuelve el usuario que acumula más horas jugadas para el género dado y una lista de la acumulación de horas jugadas por año de lanzamiento</li>
                <li><strong>best_developer_year</strong>(year - ej: 2015) devuelve el top 3 de desarrolladores con juegos MÁS recomendados por usuarios para el año dado</li>
